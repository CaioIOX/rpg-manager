...
func (r *DocumentRepository) Create(ctx context.Context, document *model.Document) error {
	query := `INSERT INTO documents (campaign_id, folder_id, template_id, title, content, is_spoiler, created_by, doc_type) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id, created_at, updated_at`
	...
	return r.db.QueryRow(ctx, query, document.CampaignID, document.FolderID, document.TemplateID, document.Title, content, document.IsSpoiler, document.CreatedBy, docType).Scan(&document.ID, &document.CreatedAt, &document.UpdatedAt)
}

func (r *DocumentRepository) GetByID(ctx context.Context, id string) (*model.Document, error) {
	doc := &model.Document{}
	query := `
		SELECT id, campaign_id, folder_id, template_id, title, content, yjs_state, doc_type, is_spoiler, created_by, created_at, updated_at
		FROM documents
		WHERE id = $1`

	err := r.db.QueryRow(ctx, query, id).Scan(&doc.ID, &doc.CampaignID, &doc.FolderID, &doc.TemplateID, &doc.Title, &doc.Content, &doc.YjsState, &doc.DocType, &doc.IsSpoiler, &doc.CreatedBy, &doc.CreatedAt, &doc.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func (r *DocumentRepository) GetByCampaign(ctx context.Context, campaignID string) ([]model.DocumentSummary, error) {
	query := `SELECT id, title, folder_id, template_id, doc_type, is_spoiler, created_by, updated_at FROM documents WHERE campaign_id = $1 ORDER BY updated_at DESC`
	...
	for rows.Next() {
		...
		err := rows.Scan(&d.ID, &d.Title, &d.FolderID, &d.TemplateID, &d.DocType, &d.IsSpoiler, &d.CreatedBy, &d.UpdatedAt)
		if err != nil {
			return nil, err
		}
		docs = append(docs, d)
...
func (r *DocumentRepository) GetByFolder(ctx context.Context, folderID string) ([]model.DocumentSummary, error) {
	query := `SELECT id, title, folder_id, template_id, doc_type, is_spoiler, created_by, updated_at
		FROM documents 
		WHERE folder_id = $1 
		ORDER BY title`
...
func (r *DocumentRepository) SearchByTitle(ctx context.Context, campaignID, query string) ([]model.DocumentSummary, error) {
	sqlQuery := `SELECT id, title, folder_id, template_id, doc_type, is_spoiler, created_by, updated_at FROM documents WHERE campaign_id = $1 AND title ILIKE '%' || $2 || '%' ORDER BY title LIMIT 20`
